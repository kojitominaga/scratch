...
def mylakeinput(origindir, outdir, affix='NORA10_11km_interpolated'):
    if not os.path.exists(outdir): os.makedirs(outdir)

    filelist = os.listdir(origindir)
    file1H = [f for f in filelist if '_DM_' in f]
    file3H = [f for f in filelist if '_DM_' in f]
    file1H.sort()
    file3H.sort()
    ...
    spacer4 = np.repeat([1], repeats = ndays).reshape((ndays, 1))
    ...
    for location, interpmethod, f1H, f3H in names:    
        ...
        wss_10m = np.maximum(wss_10m, spacer)
        pr = pr.reshape((ndays, 1)) * (60 * 60 * 24)
        pr = np.maximum(pr, spacer)
        clt = clt.reshape((ndays, 1))
        clt = np.maximum(clt, spacer)
        clt = np.minimum(clt, spacer4)
