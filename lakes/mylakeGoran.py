...
def runlake(modelid, scenarioid, eh, depth, area, longitude, latitude, secchi):
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    initp = os.path.join(outdir, 'Feb2015initGoran')
    parp = os.path.join(outdir, 'Feb2015parGoran')
    inputp = os.path.join(outdir, 'vanillainputGoran')
    if os.path.exists(os.path.join(outdir, 'Feb2015GoranREDOCOMPLETE')):
        print('lake %s is already completed' % eh)
        ret = 0
    else:
        mylakeinit(depth, area, initp)
        mylakepar(longitude, latitude, parp, atten_coeff = 1.7 / secchi)
        mylakeinput(pA, pB, datesA, datesB, eh, inputp)
        cmd = 'octave mylakeGoran.m %s %s %s %s %s %s' % (
            initp, parp, inputp, y1A - 2, y2B, outdir)
        print(cmd)
        os.system(cmd)
        for f in [initp, parp, inputp]:
            os.system('bzip2 -f %s' % f)
        expectedfs = ['Feb2015initGoran.bz2', 'Feb2015parGoran.bz2',
                      'vanillainputGoran.bz2', 'TztGoran.csv.gz', 
                      'KztGoran.csv.gz', 'HisGoran.csv.gz', 'QstGoran.csv.gz']
        flags = [os.path.exists(os.path.join(outdir, f)) for f in expectedfs]
        if all(flags):
            with open(os.path.join(outdir, 'Feb2015GoranREDOCOMPLETE'), 'w') as f:
