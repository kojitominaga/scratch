...
## runs FLake and does not calculate LakeAnalyzer indices
## (wait for any development there)

runbydepth <- function(dirname, thisdepth) {
  cat(dirname)
  cat('\n')
  fnameaffix <- 'simple'

  dirname2 <- paste0(dirname, '/', as.character(thisdepth), '/')

  if (!file.exists(dirname2)) dir.create(dirname2)
  ...
  parameters <- c(thisdepth, 1000, 0.5,
  ...
  if (!file.exists(paste0(dirname2, '/csv'))) dir.create(paste0(dirname2, '/csv'))
  
  c <- bzfile(paste0(dirname2, '/csv/dm_', fnameaffix, '.csv.bz2'),
              open = 'wb')
  ...
  c <- bzfile(paste0(dirname2, '/csv/typicalyear_', fnameaffix, '.csv.bz2'),
              open = 'wb')
...
for (fi in 1:length(folders)) {
  ...
  for (di in 1:length(depths)) {
    thisdepth <- depths[di]
    cat(paste('-- Running depth', thisdepth, '\n'))
    tryCatch({
      runbydepth(folder, thisdepth)
    }, error = function(msg) {
