...
for (i in 1:n) {
  ...
  proj4string(r) <- llCRS
  ## MAIN PURPOSE: now use r to estimate v at newloc ## 
  ...
  if (lh) {
    ...
    vfl3 <- NA ; vfl3o <- NA
    vfe3 <- NA ; vfe3o <- NA
    k3an <- NA ; i3an <- i1a ; i3an.var <- NA
    k3ao <- NA ; i3ao <- i1a ; i3ao.var <- NA
    k3bn <- NA ; i3bn <- i1a ; i3bn.var <- NA
    k3bo <- NA ; i3bo <- i1a ; i3bo.var <- NA
    k3cn <- NA ; i3cn <- i1a ; i3cn.var <- NA
    k3co <- NA ; i3co <- i1a ; i3co.var <- NA
  } else {
    ## 1b: mean of nearest four
    i1b <- mean(r[['v']][ranks <= 4])

    ## 1c: triangular 2D linear interpolation
    n1 <- r[ranks == 1, ] ; n2 <- r[ranks == 2, ] ; n3 <- r[ranks == 3, ]
    cx <- coordinates(newloc)[1, 1] ; cy <- coordinates(newloc)[1, 2]
    n1x <- coordinates(n1)[1, 1] ; n1y <- coordinates(n1)[1, 2]
    n2x <- coordinates(n2)[1, 1] ; n2y <- coordinates(n2)[1, 2]
    n3x <- coordinates(n3)[1, 1] ; n3y <- coordinates(n3)[1, 2]
    ## now say,
    ## p1|n2(x, y) - n1(x, y)| + p2|n3(x, y) - n1(x, y)| = c(x, y)
    ...
    out <- n1[['v']] + p1 * (n2[['v']] - n1[['v']]) +
      p2 * (n3[['v']] - n1[['v']])
    attr(out, "names") <- NULL                
    i1c <- out

    ## variograms 
    v3 <- variogram(v ~ 1, r, boundaries = boundaries)
    v3o <- variogram(v ~ orog, r, boundaries = boundaries)
    mg <- max(v3[['gamma']]) ; mgo <- max(v3o[['gamma']])
    md <- max(v3[['dist']]) ; mdo <- max(v3o[['dist']])
  ...
  results[i, ] <-
    c(i1a, i1b, i1c, i3an, i3ao, i3bn, i3bo, i3cn, i3co, lh * 1, mg, mgo, md, mdo,
      round(ifelse(lh, NA, vfl3[['psill']]) / mg, digits = 3),
      round(ifelse(lh, NA, vfl3o[['psill']]) / mgo, digits = 3),
      round(ifelse(rep(lh, times = 6), rep(NA, times = 6), vfe3[['psill']]) / mg, digits = 3), ## 6 elements
